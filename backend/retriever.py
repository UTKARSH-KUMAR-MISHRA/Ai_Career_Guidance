...
def retrieve_multiple(collections, query, top_k=3):
    safe_print("\n" + "="*90)
    safe_print(f"[RAG PIPELINE] STAGE 2: DOCUMENT RETRIEVAL & GROUNDED SEARCH")
    ...
    # 1. Instant Relational Database Grounded Retrieval (0.001s)
    try:
    ...
    # 2. Vector Store ChromaDB Retrieval (if extra context needed)
    if len(documents) < 3:
